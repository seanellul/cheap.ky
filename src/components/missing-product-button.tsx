...
export function MissingProductButton({ variant = "block" }: MissingProductButtonProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger>
        {variant === "inline" ? (
          <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors cursor-pointer">
            <HelpCircle className="h-3.5 w-3.5" />
            Can&apos;t find a product?
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-primary transition-colors cursor-pointer px-3 py-1.5">
